...
def add_bullets(slide, items, left, top, width, height, size=16, color=TEXT, line_spacing=1.25, bullet_char="—"):
    tb = slide.shapes.add_textbox(left, top, width, height)
    tf = tb.text_frame
    tf.word_wrap = True
    tf.margin_left = tf.margin_right = 0
...
def footer(slide, pageno, total=15):
    add_text(slide, "IonShield", Inches(0.5), Inches(7.05), Inches(4), Inches(0.3), size=9, color=MUTED)
    add_text(
        slide,
        f"{pageno} / {total}",
        Inches(12.3),
...
def std_slide(title, eyebrow=None):
    ...
    if eyebrow:
        add_text(
            s, eyebrow.upper(), Inches(0.6), Inches(0.45), Inches(10), Inches(0.3), size=10, bold=True, color=ACCENT
        )
    add_text(s, title, Inches(0.6), Inches(0.75), Inches(12), Inches(0.7), size=30, bold=True, color=TEXT)
...
add_text(s, "IonShield", Inches(0.6), Inches(2.6), Inches(12), Inches(1.4), size=72, bold=True, color=TEXT)
add_text(
    s,
...
# ── Slide 2: Problem ─────────────────────────────────────────────────────────
s = std_slide("GPS, HF, and SATCOM degrade silently — and operators find out mid-mission.", eyebrow="Problem")
add_bullets(
    s,
    [
...
pillars = [
    ("Ingest", ["NOAA SWPC, NASA OMNI, GFZ Potsdam", "5-minute cadence", "SHA-256 provenance on every input"]),
    (
        "Model",
        [
...
for x, (h1, items) in zip(xs, pillars):
    add_panel(s, x, y, w, h)
    add_text(s, h1, x + Inches(0.3), y + Inches(0.3), w - Inches(0.6), Inches(0.5), size=18, bold=True, color=ACCENT)
    add_bullets(s, items, x + Inches(0.3), y + Inches(1.0), w - Inches(0.6), h - Inches(1.2), size=13, color=TEXT)
footer(s, 5)
add_notes(
    s,
...
for i, (name, sub) in enumerate(verticals):
    ...
    add_text(
        s, name, x + Inches(0.25), y + Inches(0.2), cw - Inches(0.5), Inches(0.4), size=14, bold=True, color=ACCENT
    )
    add_text(s, sub, x + Inches(0.25), y + Inches(0.7), cw - Inches(0.5), Inches(0.8), size=11, color=MUTED)
footer(s, 8)
add_notes(
    s,
...
for hh, cw in zip(headers, col_widths):
    add_text(
        s, hh, cx + Inches(0.15), y + Inches(0.12), cw - Inches(0.2), Inches(0.4), size=12, bold=True, color=ACCENT
    )
    cx += cw
rows = [
...
add_text(s, "Jacob West — Founder", Inches(0.6), Inches(1.9), Inches(12), Inches(0.5), size=20, bold=True, color=TEXT)
add_bullets(
    s,
    [
...
add_text(s, "Round", left_x + Inches(0.3), col_y + Inches(0.25), col_w, Inches(0.4), size=14, bold=True, color=ACCENT)
add_bullets(
    s,
    [
        "Raising — pre-seed",
...
add_text(
    s, "Use of funds", right_x + Inches(0.3), col_y + Inches(0.25), col_w, Inches(0.4), size=14, bold=True, color=ACCENT
)
add_bullets(
    s,
    [
...
add_text(
    s,
    "jacob@ionshield  ·  ionshield platform live  ·  pilots in flight Q3",
    Inches(0.6),
    Inches(6.7),
    Inches(12),
    Inches(0.4),
