...
from app.models import auto_pilot, ml_classifier as mlc, retrain as retrain_module
...
async def test_archive_returns_no_aged_rows(memory_db, monkeypatch):
    ...
    monkeypatch.setattr(settings, "foundry_token",
                        type(settings.foundry_token)("T"))
    ...
    await feedback_store.record_sample(
        features=[0]*7, rule_label="BACKGROUND", ml_label="BACKGROUND",
        ml_confidence=0.9,
    )
...
async def test_archive_uploads_and_deletes(memory_db, monkeypatch):
    ...
    sid = await feedback_store.record_sample(
        features=[1.0]*7, rule_label="BACKGROUND", ml_label="BACKGROUND",
        ml_confidence=0.95,
    )
...
async def test_register_active_then_challenger(memory_db):
    await feedback_store.register_model_version(
        ...
        train_accuracy=0.9, artifact_path="/a.json",
        activate=True, challenger=False,
    )
    await feedback_store.register_model_version(
        version="logreg-v3", n_train=200, n_real_samples=50,
        train_accuracy=0.92, artifact_path="/b.json",
        activate=False, challenger=True,
    )
...
async def test_register_model_version_rejects_active_and_challenger(memory_db):
    with pytest.raises(ValueError):
        await feedback_store.register_model_version(
            version="x", n_train=1, n_real_samples=1,
            train_accuracy=0.9, artifact_path="/x",
            activate=True, challenger=True,
        )
...
async def test_auto_retrain_skipped_when_drift_within_threshold(memory_db):
    ...
    for _ in range(250):
        await feedback_store.record_sample(
            features=[0]*7, rule_label="BACKGROUND",
            ml_label="BACKGROUND", ml_confidence=0.95,
        )
...
async def test_auto_retrain_triggers_when_drift_below_threshold(
    ...
    for cls in mlc.CLASSES:
        for _ in range(60):
            await feedback_store.record_sample(
                features=mlc._synth_sample(rng, cls), rule_label=cls,
                ml_label="BACKGROUND",  # always wrong → low agreement
                ml_confidence=0.6,
            )
    monkeypatch.setattr(mlc, "ARTIFACT_PATH", tmp_path / "live.json")

    # Reset cooldown (module-level state from earlier tests)
    auto_pilot._last_retrain_at = 0.0
